...
def extract_frame(input_video_path, target_fold, extract_frame_num=10):
    ...
    for i in range(extract_frame_num):
        ...
        print(
            "Extract frame {:d} from original frame {:d}, total video frame {:d} at frame rate {:d}.".format(
                i, frame_idx, total_frame_num, int(fps)
            )
        )
        ...
        save_image(
            image_tensor, target_fold + "/frame_{:d}/".format(i) + video_id + ".jpg"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Should be a csv file of a single columns, each row is the input video path.",
    )
    parser.add_argument(
        "-target_fold",
        type=str,
    ...
    for file_id in range(num_file):
        try:
            print(
                "processing video {:d}: {:s}".format(file_id, input_filelist[file_id])
            )
